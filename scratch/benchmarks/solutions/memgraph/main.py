...
class MemgraphJourneyManager(JourneyManager):
    ...
    def setup(self):
        with self.driver.session() as session:
            try:
                session.run("CREATE INDEX ON :Event(id)")
                session.run("CREATE INDEX ON :Event(status)")
                session.run("CREATE INDEX ON :Journey(id)")
            except:
...
if __name__ == "__main__":
    ...
    print("Starting Benchmark: Memgraph")
