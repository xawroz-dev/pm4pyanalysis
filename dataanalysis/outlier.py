...
outliers_category_A = np.random.choice(['Yellow', 'Green'], size=num_outliers, p=[0.7, 0.3]) # More yellow/green
...
for col in ['category_A', 'category_B', 'category_C']:
    le = LabelEncoder()
    df_if[col] = le.fit_transform(df_if[col])
...
df_if_scaled[[f'num_feat_{i+1}' for i in range(num_features)]] = scaler_if.fit_transform(df_if_scaled[[f'num_feat_{i+1}' for i in range(num_features)]])
...
if_true_positives = df_if_scaled[(df_if_scaled['is_outlier_true'] == 1) & (df_if_scaled['if_outlier_predicted'] == 1)].shape[0]
if_false_positives = df_if_scaled[(df_if_scaled['is_outlier_true'] == 0) & (df_if_scaled['if_outlier_predicted'] == 1)].shape[0]
if_false_negatives = df_if_scaled[(df_if_scaled['is_outlier_true'] == 1) & (df_if_scaled['if_outlier_predicted'] == 0)].shape[0]

print(f"Isolation Forest - True Positives: {if_true_positives}, False Positives: {if_false_positives}, False Negatives: {if_false_negatives}")
...
df_md_scaled['md_distance'] = [mahalanobis(x, mean_vector, covariance_matrix_inv) for x in df_md_scaled.drop('is_outlier_true', axis=1).values]
...
md_true_positives = df_md_scaled[(df_md_scaled['is_outlier_true'] == 1) & (df_md_scaled['md_outlier_predicted'] == 1)].shape[0]
md_false_positives = df_md_scaled[(df_md_scaled['is_outlier_true'] == 0) & (df_md_scaled['md_outlier_predicted'] == 1)].shape[0]
md_false_negatives = df_md_scaled[(df_md_scaled['is_outlier_true'] == 1) & (df_md_scaled['md_outlier_predicted'] == 0)].shape[0]

print(f"Mahalanobis Distance - True Positives: {md_true_positives}, False Positives: {md_false_positives}, False Negatives: {md_false_negatives}")
...
df_if_scaled['IF_Prediction'] = df_if_scaled['if_outlier_predicted'].map({0: 'Predicted Inlier', 1: 'Predicted Outlier'})

df_md_scaled['True_Status'] = df_md_scaled['is_outlier_true'].map({0: 'Inlier', 1: 'True Outlier'})
df_md_scaled['MD_Prediction'] = df_md_scaled['md_outlier_predicted'].map({0: 'Predicted Inlier', 1: 'Predicted Outlier'})
...
plt.subplot(1, 2, 1)
sns.scatterplot(x=df_if_scaled['num_feat_1'], y=df_if_scaled['num_feat_2'], hue=df_if_scaled['True_Status'], style=df_if_scaled['IF_Prediction'], palette='coolwarm', s=50, alpha=0.7)
...
plt.subplot(1, 2, 2)
sns.scatterplot(x=df_md_scaled['num_feat_1'], y=df_md_scaled['num_feat_2'], hue=df_md_scaled['True_Status'], style=df_md_scaled['MD_Prediction'], palette='coolwarm', s=50, alpha=0.7)
